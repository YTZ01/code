...
class LocalHyperGATlayer(nn.Module):
    def __init__(self, dim, layer, alpha, bias=False, act=True):
        ...
        # Parameters 
        # node->edge->node
        self.w1 = Parameter(torch.Tensor(self.dim, self.dim)) #200,200
        self.w2 = Parameter(torch.Tensor(self.dim, self.dim))
  
        self.a10 = nn.Parameter(torch.Tensor(size=(self.dim, 1)))  #200,1 初始化
    ...
    def forward(self, hidden, H, s_c):
        ...
        N = H.shape[1]            # node num 19
        edge_num = H.shape[2]     # edge num 70
        H_adj = torch.ones_like(H)
        mask = torch.zeros_like(H)
        H_adj = torch.where(H>0, H_adj, mask)
        s_c = s_c.expand(-1, N, -1)
        h_emb = hidden #[64,19,200]
        h_embs = []
        
        for i in range(self.layer):
            edge_cluster = torch.matmul(H_adj.transpose(1,2), h_emb)  #[64,70,200]                # (Batchsize, edge_num, latent_dim)
            h_t_cluster = h_emb + s_c#[64,19,200]
            ...
            feat = edge_c_in * h_4att0 #[64,19,70,200]
            ...
            edge = torch.matmul(alpha1.transpose(1,2), h_emb)   #[64,70,200]                      # (Batchsize, edge_num, latent_dim)
            ...
            feat_e2n = edge_in * h_4att1 #[64,19,70,200]
            ...
            alpha2 = torch.where(H.eq(1), atts20, zero_vec) #[64,19,70]
            alpha2 = torch.where(H.eq(2), atts21, alpha2) #[64,19,70]
            alpha2 = torch.where(H.eq(3), atts22, alpha2)#[64,19,70]
            
            alpha2 = F.softmax(alpha2, dim=2)                                         # (Batchsize, N, edge_num)

            h_emb = torch.matmul(alpha2, edge)                                        # (Batchsize, N, latent_dim)
            h_embs.append(h_emb) #[64,19,200]

        h_embs = torch.stack(h_embs, dim=1) #[64,1,19,200]
        h_out = torch.sum(h_embs, dim=1) #[64,19,200]
